...
AUDIT_LOGGING_DYNAMODB_EVENTS_TABLE="bichard-7-${WORKSPACE}-audit-log-events"
AUDIT_LOGGING_DYNAMODB_REGION="$AWS_REGION"
AUDIT_LOGGING_API_ID=$(aws apigateway get-rest-apis --query "items[0].id" --output text)
AUDIT_LOGGING_API_URL="https://${AUDIT_LOGGING_API_ID}.execute-api.eu-west-2.amazonaws.com/${WORKSPACE}"
AUDIT_LOGGING_API_KEY=$(aws apigateway get-api-keys --include-values --query "items[0].value" --output text)
...
echo "export AUDIT_LOGGING_DYNAMODB_EVENTS_TABLE=\"${AUDIT_LOGGING_DYNAMODB_EVENTS_TABLE}\"" >> $TEST_ENV_FILE
echo "export AUDIT_LOGGING_DYNAMODB_REGION=\"${AUDIT_LOGGING_DYNAMODB_REGION}\"" >> $TEST_ENV_FILE
echo "export AUDIT_LOGGING_API_URL=\"${AUDIT_LOGGING_API_URL}\"" >> $TEST_ENV_FILE
